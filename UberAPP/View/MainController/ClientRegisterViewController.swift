            ...
            if passworddField.text?.count ?? 0 < 6 {
                ...
                if phoneNumberField.text?.count ?? 0 != 9 {
                    ...
                    if locationAllowed == true {
                        clientVM.registerAsClient(name: self.nameTextField.text!, phoneNumber: self.phoneNumberField.text!, password: self.passworddField.text!, latitude: 1234567, longitude: 2343777) { user, status in
                            if status == true {
